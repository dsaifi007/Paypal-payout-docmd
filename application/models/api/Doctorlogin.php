    ...
    public function doctorlogin_post() {
        try {
            ...
            if (check_user_input_values($this->doctor_data)) {
                $this->_loadModel();
                $response = $this->doctor_login_model->check_email_and_pass_existence($this->doctor_data);
                //dd($response);
        ...
        try {
            ...
            if ($this->doctor_data['email'] != '') {
                ...
                if ($response == false) {
                    ...
                    $this->response_send = ["message" => $this->lang->line('user_email_sent'), "status" => $this->config->item("status_true")];
                }
            } else {
                $this->response_send = ["message" => $this->lang->line('all_field_required'), "status" => $this->config->item("status_false")];
            }
