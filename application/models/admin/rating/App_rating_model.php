...
class App_rating_model extends CI_Model {
    ...
    function _get_order_list_query($post) {
        // 2 show the app rating provided by the doctor
        if ($post['id'] == 2 && $post['id'] != null) {
            // we will show the doctor data
            $this->db->select("app_rating.id,doctors.first_name,doctors.last_name, app_rating.rating,app_rating.review,app_rating.created_at");
            $this->db->from("app_rating");
            $this->db->join("doctors", "app_rating.doctor_id=doctors.id", "INNER");
        } else {
            $this->db->select("app_rating.id,patient_info.first_name,patient_info.last_name, app_rating.rating,app_rating.review,app_rating.created_at");
            $this->db->from("app_rating");
            $this->db->join("user_patient", "user_patient.user_id=app_rating.user_id", "INNER");
