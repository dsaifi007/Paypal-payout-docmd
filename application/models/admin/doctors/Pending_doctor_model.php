...
class Pending_doctor_model extends CI_Model {
    ...
    function _get_order_list_query($post) {
        ...
        $this->db->join("language", "doctors.language_id = language.language_id", "INNER");
        $this->db->where("status", $this->doctor_pending_status);
        //$this->db->get();
    ...
    public function get_all_manual_email_template() {
        $this->db->where_in("type", ["accept", "reject", "other_emails"]);
        $this->db->where("email_event IS NULL");
        $this->db->select("id,subject,message");
