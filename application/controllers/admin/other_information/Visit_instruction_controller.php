...
class Visit_instruction_controller extends MY_Controller {

    protected $data = [];
    protected $language_file = "other_information/visit_instruction";
    protected $model = 'admin/other_information/visit_instruction_model';
    protected $is_model = "visit_instruction_model";
    ...
    public function index() {
        $this->data['items'] = $this->{$this->is_model}->get_all_visit_instruction_list();
        ...
        $this->data['view'] = "admin/other_information/visit_instruction/visit_instruction_view";
        $this->data['add_datatable_js'] = "other_information/visit_instruction.js";
    ...
    public function add_visit_instruction_info() {
        $this->data['page_title'] = $this->lang->line("add_title");
        if (isset($_FILES['visit_instruction_csv']['name']) && $_FILES['visit_instruction_csv']['name'] != '') {
            $response = $this->visit_instruction_csv_upload($_FILES);
            if (isset($response['error'])) {
                $this->data['error'] = $response['error'];
            }else{
                $result = insertCsvData($response,"visit_instruction"); // making in helper function
            ...
            if ($this->input_validations()) {
                ...
                $this->item_id = $this->{$this->is_model}->add_and_update_visit_instruction($input_data);
                $this->session->set_flashdata("flashsuccess", $this->lang->line("success_visit_instruction_save"));
                redirect("admin/other_information/visit_instruction_controller/edit_visit_instruction_info/" . $this->item_id);
            }
        }
        $this->BuildFormEnv(["template_helper"]);
        $this->data['add_datatable_js'] = "other_information/visit_instruction.js";
        $this->data['view'] = "admin/other_information/visit_instruction/add_visit_instruction_view";
        $this->displayview($this->data);
    }

    private function visit_instruction_csv_upload($file, $id = null) {
        
            $this->load->library("common");
            $this->load->helper('string');
            $rename_image = (random_string('numeric') + time()) . random_string();
            $img_data = $this->common->file_upload("assets/admin/img/other_information", "visit_instruction_csv", $rename_image,$fileformat = "csv");
...
//                }
    ...
    public function edit_visit_instruction_info($id) {
        $this->data['page_title'] = $this->lang->line("edit_title");
        $this->data['items'] = $this->{$this->is_model}->get_visit_instruction_info($id);
        $this->data['success'] = ($this->session->flashdata("flashsuccess")) ? $this->session->flashdata("flashsuccess") : '';

        if ($this->input_validations($id, $this->data['items'])) {
            $input_data = $this->security->xss_clean($this->input->post());
            $this->{$this->is_model}->add_and_update_visit_instruction($input_data, $id);
            $this->data['success'] = $this->lang->line("success_visit_instruction_edit_save");
        }
        $this->BuildFormEnv(["template_helper"]);
        $this->data['view'] = "admin/other_information/visit_instruction/edit_visit_instruction_view";
        $this->displayview($this->data);
    }
