...
class Allergies_controller extends MY_Controller {

    protected $data = [];
    protected $language_file = "other_information/allergies";
    protected $model = 'admin/other_information/allergies_model';
    protected $is_model = "allergies_model";
    ...
    public function index() {
        $this->data['items'] = $this->{$this->is_model}->get_all_allergies();
        ...
        $this->data['view'] = "admin/other_information/allergies/allergies_view";
        $this->data['add_datatable_js'] = "other_information/allergies.js";
                $this->data['active_class1'] = "active";
        $this->data['open6'] = "open";
    ...
    /*
      -------------------------------------------------------------------------------------------
      |		Work -- Add new allergies
      |		@return -- view
      -------------------------------------------------------------------------------------------
     */

    public function add_allergy_info() {
        $this->data['page_title'] = $this->lang->line("add_title");


        if (isset($_FILES['allergy_csv']['name']) && $_FILES['allergy_csv']['name'] != '') {
            $response = $this->allergy_csv_upload($_FILES);
            if (isset($response['error'])) {
                $this->data['error'] = $response['error'];
            }else{
                $result = insertCsvData($response,"allergies"); // making in helper function
            ...
            if ($this->input_validations()) {
                ...
                $this->item_id = $this->{$this->is_model}->add_and_update_allergy($input_data);
                $this->session->set_flashdata("flashsuccess", $this->lang->line("success_allergy_save"));
                redirect("admin/other_information/allergies_controller/edit_allergy_info/" . $this->item_id);
            }
        }
        $this->BuildFormEnv(["template_helper"]);
        $this->data['add_datatable_js'] = "other_information/allergies.js";
        $this->data['view'] = "admin/other_information/allergies/add_allergy_view";
         $this->data['active_class1'] = "active";
        $this->data['open6'] = "open";
        $this->displayview($this->data);
    }

    private function allergy_csv_upload($file, $id = null) {
        
            $this->load->library("common");
            $this->load->helper('string');
            $rename_image = (random_string('numeric') + time()) . random_string();
            $img_data = $this->common->file_upload("assets/admin/img/other_information", "allergy_csv", $rename_image,$fileformat = "csv");
    ...
    public function edit_allergy_info($id) {
        $this->data['page_title'] = $this->lang->line("edit_title");
        $this->data['items'] = $this->{$this->is_model}->get_allergy_info($id);
        $this->data['success'] = ($this->session->flashdata("flashsuccess")) ? $this->session->flashdata("flashsuccess") : '';

        if ($this->input_validations($id, $this->data['items'])) {
            $input_data = $this->security->xss_clean($this->input->post());
            $this->{$this->is_model}->add_and_update_allergy($input_data, $id);
            $this->data['success'] = $this->lang->line("success_allergy_edit_save");
        }
        $this->BuildFormEnv(["template_helper"]);
        $this->data['view'] = "admin/other_information/allergies/edit_allergy_view";
         $this->data['active_class1'] = "active";
        $this->data['open6'] = "open";
        $this->displayview($this->data);
    }
    ...
    public function delete($id) {    
       $this->db->where('id', $id);
       $this->db->update("allergies",["is_deleted"=>1]); 
       $this->session->set_flashdata("flashsuccess", $this->lang->line("delete_allergy"));
       redirect("admin/other_information/allergies_controller/index");
    }
